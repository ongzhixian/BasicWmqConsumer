...
const int TIMEOUT_MILLISECONDS = 30 * 1000;
...
const string WMQ_APPLICATIONNAME_CONFIGURATION_KEY = "IBM_MQ:WMQ_APPLICATIONNAME";
...
using (IMessageConsumer? consumer = sessionWMQ.CreateConsumer(destination))
{
    connection.Start();

    while (true)
    {
        Console.WriteLine("Receive message: " + TIMEOUT_MILLISECONDS / 1000 + " seconds wait time");

        // Wait for 30 seconds for messages. Exit if no message by then
        var textMessage = (ITextMessage)consumer.Receive(TIMEOUT_MILLISECONDS);
        if (textMessage != null)
        {
